    ...
    */
    ...
    accounts_func_small: {
      ...
      optionsFunction: async (values: any) => {
        const data = await API.requestRecords(
          'accounts',
          [['contract_type', '=', values.accounts_func_big]],
          ['_id', 'name'],
          { pageSize: '10' },
        );
    ...
    city: {
      ...
      optionsFunction: (values: any) => {
        const cityData = {
          bj: [
            { label: '东城区', value: 'bj-1' },
            { label: '西城区', value: 'bj-2' },
          ],
          sh: [
            { label: '松江', value: 'sh-1' },
            { label: '浦东', value: 'sh-2' },
    ...
    accounts_small: {
      ...
      optionsFunction: (values: any) => {
        const smallData = {
          one: [
            { label: '11合同', value: 'one-1' },
            { label: '12合同', value: 'one-2' },
          ],
          two: [
            { label: '21合同', value: 'two-1' },
            { label: '22合同', value: 'two-2' },
          ],
          three: [
            { label: '31合同', value: 'three-1' },
            { label: '32合同', value: 'three-2' },
          ],
        };
    ...
    accounts_refeto_small: {
      ...
      optionsFunction: async (values: any) => {
        const data = await API.requestRecords(
          'accounts',
          [['contract_type', '=', values.accounts_refeto_big]],
          ['_id', 'name'],
          { pageSize: '10' },
        );
        const results = data.value.map((item: any) => {
    ...
    accounts_re_small: {
      ...
      optionsFunction: async (values: any) => {
        const data = await API.requestRecords(
          'accounts',
          [['contract_type', '=', values.accounts_re_big]],
          ['_id', 'name'],
          { pageSize: '10' },
        );
        const results = data.value.map((item: any) => {
